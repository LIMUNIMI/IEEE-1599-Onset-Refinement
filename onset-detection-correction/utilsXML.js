export const fetchXMLOnsets = doc => {

  const trackEventsByTime = doc.tracks[TRACK_FILE_NAME_XML].trackEventsByTime;
...
export const editXML = (docXML, correctedTimes) => {
  ...
  for (let t = 0; t < tracks.length; t++) {
    if (tracks[t].nodeType == 1) {
      if (tracks[t].getAttributeNode("file_name").nodeValue == TRACK_FILE_NAME_XML) {
