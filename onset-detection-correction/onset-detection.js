onsetDetection = inputReal => {
	...
	//log.innerHTML += "... computing STFT<br>"
	let res = STFT(inputReal);

	console.info('... computing complex domain detection function');
	//log.innerHTML += "... computing complex domain detection function<br>"
	let df = createDetectionFunction(res);

	console.info('... computing percussive feature detection function');
	//log.innerHTML += "... computing percussive feature detection function<br>"
	let percussiveFeature = percussiveFeatureDetection(res);

	// normalizing detection functions normalized in range [0,1]
	df = scale(1/max(df), df);
	percussiveFeature = scale(1/max(percussiveFeature), percussiveFeature);

	if (showPlots) plotData1(inputReal, df, percussiveFeature); 

	console.info('... computing global detection function');
	//log.innerHTML += "... computing global detection function<br>";
	...
	df = add(scale(weight, df), scale((1-weight), percussiveFeature));

	if (showPlots) plotData1b(inputReal, df);
...
peakPicking = df => {
	...
	// !! quei delta non vanno bene perchè io ho normalizzato df fra 0 e 1
	const delta = 0.1;
	...
	if (showPlots) plotData2(df, threshold);	// plot df with threshold
